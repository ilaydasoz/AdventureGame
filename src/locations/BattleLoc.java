...
public class BattleLoc extends Location {
    ...
    public boolean onLocation() {
        ...
        if (this.getName() == "Mine") {
            int min = 3;
            int max = this.getMaxObstacle();
            obsNumber = (int) Math.floor(Math.random()*(max-min+1)+min) ; // random number of snakes between 3 & 6
        }
        ...
        if (selectCase.equals("F")) {
            if (combat(obsNumber)) {
                System.out.println("All obstacles are killed!");
                if (this.award.equals("Food") && getPlayer().getInventory().isFood() == false) {
                    this.getPlayer().getInventory().setFood(true);
                    System.out.println("You have won " + this.award + "!");
                }

                if (this.award.equals("Firewood") && getPlayer().getInventory().isFirewood() == false) {
                    this.getPlayer().getInventory().setFirewood(true);
                    System.out.println("You have won " + this.award + "!");
                }

                if (this.award.equals("Water") && getPlayer().getInventory().isWater() == false) {
                    ...
                    while (this.getPlayer().getHealth() > 0 && this.getObstacle().getHealth() > 0) {
                        ...
                        if (selectCombat.equals("H") ){
                            ...
                            if (this.getObstacle().getHealth() > 0) {
                                System.out.println();
                                System.out.println(this.getObstacle().getName() + " HIT YOU!");
                                obstacleDamage = this.getObstacle().getHealth() - this.getPlayer().getTotalDamage();
                                if (obstacleDamage < 0) {
                                    obstacleDamage = 0;
                                }
                                this.getPlayer().setHealth(this.getPlayer().getHealth() - obstacleDamage);
